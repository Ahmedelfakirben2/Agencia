...
app.use(session({
  secret: process.env.SESSION_SECRET || 'tu_clave_secreta_temporal', // Usa una variable de entorno
...
// Middleware para variables locales y ofertas en todas las páginas (después de la sesión)
app.use(async (req, res, next) => {
    ...
    // Obtener ofertas para mostrar en todas las páginas
    try {
        const { Oferta } = await import('./models/Oferta.js');
        const ofertasGlobales = await Oferta.findAll({ limit: 3 });
        res.locals.ofertasGlobales = ofertasGlobales;
    } catch (error) {
        console.error('Error al cargar ofertas globales:', error);
        res.locals.ofertasGlobales = [];
    }
...
const ofertasDir = path.join(uploadsDir, 'ofertas');

[uploadsDir, viajesDir, guiasDir, hotelesDir, ofertasDir].forEach(dir => {
